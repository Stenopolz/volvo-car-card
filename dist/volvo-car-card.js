...
`;var d={car:`<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  </svg>`};class g extends HTMLElement{_config=null;_hass=null;_shadowAttached=!1;setConfig(r){if(!r)throw Error("Invalid configuration");if(this._config={...r},this._shadowAttached)this._render()}set hass(r){let o=this._hass;if(this._hass=r,this._shadowAttached&&this._config&&this._statesChanged(o,r))this._render()}getCardSize(){return 4}getGridOptions(){return{rows:4,columns:6,min_rows:3}}static getConfigElement(){return document.createElement("volvo-car-editor")}static getStubConfig(){return{type:"custom:volvo-car-card"}}connectedCallback(){if(!this.shadowRoot)this.attachShadow({mode:"open"});this._shadowAttached=!0,this._render()}disconnectedCallback(){this._shadowAttached=!1}_render(){if(!this.shadowRoot)return;if(!this._config){this.shadowRoot.innerHTML=`<style>${i}</style>
        <div class="card-root"><div class="error-box">No configuration provided.</div></div>`;return}let{battery_entity:r,range_entity:o,lock_entity:a,odometer_entity:x,name:p}=this._config,b=this._getState(r),f=this._getState(o),n=this._getState(a),h=this._getState(x),m=n?.state==="locked",c=b?parseFloat(b.state):null,l=f?parseFloat(f.state):null,u=h?parseFloat(h.state):null,z=p??this._deriveName(n??b),t=c!==null&&c<20;this.shadowRoot.innerHTML=`
      <style>${i}</style>
      <div class="card-root">
        ...
        <div class="stats-grid">
          ${this._renderBatteryStat(c,t)}
          ${this._renderSimpleStat("Range",l,"km",d.range)}
          ${this._renderSimpleStat("Odometer",u?Math.round(u):null,"km",d.odometer)}
          ${this._renderLockStat(m,!!n)}
        </div>
    ...
    `}_renderLockStat(r,o){let a=o?r?"Locked":"Unlocked":"—",x=r?d.lock:d.unlock;return`
      <div class="stat-card">
    ...
    `}_getState(r){if(!r||!this._hass)return null;return this._hass.states[r]??null}_deriveName(r){if(!r)return"Volvo";return(r.attributes.friendly_name??"").replace(/\s+(battery.*|lock.*|odometer.*|distance.*)$/i,"").trim()||"Volvo"}_statesChanged(r,o){if(!r||!this._config)return!0;return[this._config.battery_entity,this._config.range_entity,this._config.lock_entity,this._config.odometer_entity].filter(Boolean).some((x)=>r.states[x]!==o.states[x])}_callService(r,o,a){if(!this._hass)return;this._hass.callService(r,o,{entity_id:a}).catch((x)=>{console.error(`[volvo-car-card] Service call ${r}.${o} failed:`,x)})}}class s extends HTMLElement{_config={type:"custom:volvo-car-card"};_hass;_attached=!1;set hass(r){if(this._hass=r,this._attached)this._render()}setConfig(r){if(this._config={...r},this._attached)this._render()}connectedCallback(){this._attached=!0,this.attachShadow({mode:"open"}),this._render()}disconnectedCallback(){this._attached=!1}_render(){if(!this.shadowRoot)return;let r=this._config;this.shadowRoot.innerHTML=`
      <style>
        .editor-root {
          display: flex;
          flex-direction: column;
    ...
    `,this.shadowRoot.querySelectorAll("ha-entity-picker").forEach((o)=>{o.addEventListener("value-changed",(a)=>{let x=o.dataset.field,p=a.detail.value;this._updateConfig(x,p)})}),this.shadowRoot.querySelectorAll("ha-textfield").forEach((o)=>{o.addEventListener("change",(a)=>{let x=o.dataset.field,p=a.target.value;this._updateConfig(x,p||void 0)})})}_updateConfig(r,o){let a={...this._config};if(o)a[r]=o;else delete a[r];this._config=a,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config},bubbles:!0,composed:!0}))}}customElements.define("volvo-car-card",g);customElements.define("volvo-car-editor",s);window.customCards??=[];window.customCards.push({type:"volvo-car-card",name:"Volvo Car Card",description:"A card for displaying Volvo Cars integration data and controls.",preview:!1,documentationURL:"https://github.com/home-assistant/core/tree/dev/homeassistant/components/volvo"});
