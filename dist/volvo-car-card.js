...
`;var g={car:`<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  </svg>`};class s extends HTMLElement{_config=null;_hass=null;_initialized=!1;setConfig(r){if(!r)throw Error("Invalid configuration");if(this._config={...r},this._initialized)this._render()}set hass(r){let o=this._hass;if(this._hass=r,this._initialized&&this._config&&this._statesChanged(o,r))this._render()}getCardSize(){return 4}getGridOptions(){return{rows:4,columns:6,min_rows:3}}static getConfigElement(){return document.createElement("volvo-car-editor")}static getStubConfig(r){let o={type:"custom:volvo-car-card"};if(!r)return o;let a=Object.keys(r.states),x=(p,d)=>a.find((b)=>b.startsWith(`${p}.`)&&b.endsWith(`_${d}`));return o.battery_entity=x("sensor","battery_charge_level"),o.range_entity=x("sensor","distance_to_empty_battery")??x("sensor","distance_to_empty_tank"),o.lock_entity=x("lock","lock"),o.odometer_entity=x("sensor","odometer"),Object.keys(o).forEach((p)=>{if(o[p]===void 0)delete o[p]}),o}connectedCallback(){if(!this._initialized)this.attachShadow({mode:"open"}),this._initialized=!0;this._render()}disconnectedCallback(){}_render(){if(!this.shadowRoot)return;if(!this._config){this.shadowRoot.innerHTML=`<style>${h}</style>
