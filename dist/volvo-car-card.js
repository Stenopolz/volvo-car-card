  ...
  <path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z"/>
      ...
      <div class="card-root">
        ...
        <div class="card-content">
          ...
          <!-- Primary range metric -->
          ${this._renderRangeMetric(Y,w,D,v)}

          <!-- Energy status rows -->
          <div class="status-block">
            ${k?this._renderChargeRow(b,G):""}
            ${this._renderFuelRow(C,c)}
          </div>
        ...
        <!-- Vehicle hero image -->
        <div class="hero-zone">
          ${j?`<img class="car-image" src="${j}" alt="${B}" />`:`<div class="hero-placeholder">${A}</div>`}
        </div>

        <!-- Quick-action buttons -->
        <div class="actions-bar">
          ${r?`
            <button class="circle-btn" id="btn-lock" title="${e?"Unlock":"Lock"}">
              ${e?Q:f}
            </button>
          `:""}
          ${r&&e?`
    ...
    `,this._attachHandlers(r,e,h,d,s)}_renderRangeMetric(o,p,r,l){let x=p!==null&&o!==null&&!isNaN(o),i=l!==null&&r!==null&&!isNaN(r);if(x&&i)return`
        <div class="range-metric range-metric--combined">
          ...
          <div class="range-breakdown">
            ...
            <span class="range-part">
              <span class="range-part-icon range-part-icon--electric">${n}</span>
              ${Math.round(o)}
            </span>
            <span class="range-part-unit">km</span>
          </div>
        </div>
      `;if(!p&&!l)return'<div class="range-metric"><span class="range-unavailable">— km</span></div>';let h=x?o:i?r:null;if(h===null)return'<div class="range-metric"><span class="range-unavailable">Updating…</span></div>';let d=x?n:g,s=x?"range-part-icon--electric":"range-part-icon--fuel";return`
      <div class="range-metric">
        <span class="range-value">${Math.round(h).toLocaleString()}</span>
        <span class="range-unit">km</span>
        <span class="range-part-icon ${s} range-single-icon">${d}</span>
      </div>
