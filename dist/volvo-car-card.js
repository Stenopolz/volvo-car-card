  ...
  <path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z"/>
      ...
      <div class="card-root">
        ...
        <div class="card-content">
          ...
          <!-- Primary range metric -->
          ${this._renderRangeMetric(W,w,Z,b)}

          <!-- Energy status rows -->
          <div class="status-block">
            ${$?this._renderChargeRow(v,D):""}
            ${this._renderFuelRow(X,s)}
          </div>
        ...
        <!-- Vehicle hero image -->
        <div class="hero-zone">
          ${j?`<img class="car-image" src="${j}" alt="${M}" />`:`<div class="hero-placeholder">${k}</div>`}
        </div>
    ...
    `,this._attachHandlers(r,h,e)}_renderRangeMetric(o,p,r,i){let n=p!==null&&o!==null&&!isNaN(o),x=i!==null&&r!==null&&!isNaN(r);if(n&&x)return`
        <div class="range-metric range-metric--combined">
          ...
          <div class="range-breakdown">
            <span class="range-part">
              <span class="range-part-icon range-part-icon--fuel">${d}</span>
              ${Math.round(r)}
            </span>
      ...
      `;if(!p&&!i)return'<div class="range-metric"><span class="range-unavailable">— km</span></div>';let e=n?o:x?r:null;if(e===null)return'<div class="range-metric"><span class="range-unavailable">Updating…</span></div>';let a=n?g:d,c=n?"range-part-icon--electric":"range-part-icon--fuel";return`
      <div class="range-metric">
        <span class="range-value">${Math.round(e).toLocaleString()}</span>
        <span class="range-unit">km</span>
        <span class="range-part-icon ${c} range-single-icon">${a}</span>
      </div>
    ...
    `}_renderFuelRow(o,p){if(!this._config?.fuel_entity)return"";let r=p?.attributes.unit_of_measurement??"L",i=o!==null&&!isNaN(o)?`${Math.round(o)} ${r}`:"—";return`
      <div class="status-row status-row--fuel">
        <span class="status-icon">${d}</span>
        <span class="status-pct">${i}</span>
      </div>
    `}_renderSegBar(o,p){let r=o!==null&&!isNaN(o)?Math.round(o/(100/q)):0;return`<div class="seg-bar">${Array.from({length:q},(n,x)=>`<span class="seg ${x<r?p:""}"></span>`).join("")}</div>`}_energyBgStyle(o){let p=Math.max(0,Math.min(100,o)),r=`${Math.max(0,p-6)}%`,i=`${p}%`;return`background: linear-gradient(to right,
      rgba(74,222,128,0.28) 0%,
      rgba(74,222,128,0.28) ${r},
      rgba(74,222,128,0.06) ${i},
      transparent 100%);`}_chargingLabel(o,p){if(!o){if(p!==null&&p>=100)return"Fully charged";return"Ready to drive"}let r=o.state.toLowerCase();if(r.includes("charging"))return o.state;if(r.includes("full"))return"Fully charged";if(r.includes("schedul"))return"Charge scheduled";return o.state||"Ready to drive"}_getState(o){if(!o||!this._hass)return null;return this._hass.states[o]??null}_deriveName(o){if(!o)return"Volvo";return(o.attributes.friendly_name??"").replace(/\s+(battery.*|lock.*|odometer.*|distance.*|fuel.*|charging.*|camera.*)$/i,"").trim()||"Volvo"}_statesChanged(o,p){if(!o||!this._config)return!0;return[this._config.battery_entity,this._config.range_entity,this._config.lock_entity,this._config.fuel_entity,this._config.fuel_range_entity,this._config.charging_status_entity,this._config.climate_entity,this._config.vehicle_image_entity].filter(Boolean).some((i)=>o.states[i]!==p.states[i])}_attachHandlers(o,p,r){let i=this.shadowRoot;i.getElementById("btn-lock")?.addEventListener("click",()=>{if(!o)return;let n=p?"unlock":"lock";this._callService("lock",n,o)}),i.getElementById("btn-unlock")?.addEventListener("click",()=>{if(!o)return;this._callService("lock","unlock",o)}),i.getElementById("btn-climate")?.addEventListener("click",()=>{if(!r)return;let n=r.split(".")[0],x=n==="button"?"press":"turn_on";this._callService(n,x,r)})}_callService(o,p,r){if(!this._hass)return;this._hass.callService(o,p,{entity_id:r}).catch((i)=>{console.error(`[volvo-car-card] Service call ${o}.${p} failed:`,i)})}}var F={name:"Card name (optional)",battery_entity:"Battery level entity (sensor)",range_entity:"Electric range entity (sensor)",fuel_entity:"Fuel level entity (sensor)",fuel_range_entity:"Fuel range entity (sensor)",lock_entity:"Lock entity",charging_status_entity:"Charging status entity (sensor, optional)",climate_entity:"Start climate entity (button, optional)",vehicle_image_entity:"Vehicle image entity (camera/image, optional)"},G=[{name:"name",selector:{text:{}}},{name:"battery_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"range_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"fuel_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"fuel_range_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"lock_entity",selector:{entity:{filter:[{domain:"lock"}]}}},{name:"charging_status_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"climate_entity",selector:{entity:{filter:[{domain:"button"}]}}},{name:"vehicle_image_entity",selector:{entity:{filter:[{domain:"camera"},{domain:"image"}]}}}];class z extends HTMLElement{_config={type:"custom:volvo-car-card"};_hass;_built=!1;_form;set hass(o){if(this._hass=o,this._form)this._form.hass=o}setConfig(o){if(this._config={...o},this._form)this._form.data=this._config}connectedCallback(){if(this._built)return;this._built=!0,this.attachShadow({mode:"open"});let o=this.shadowRoot;o.innerHTML=`<style>
      ha-form { display: block; }
    </style>`;let p=document.createElement("ha-form");this._form=p,p.hass=this._hass,p.data=this._config,p.schema=G,p.computeLabel=(r)=>F[r.name]??r.name,p.addEventListener("value-changed",(r)=>{let i={...r.detail.value,type:"custom:volvo-car-card"};this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}),o.appendChild(p)}}customElements.define("volvo-car-card",f);customElements.define("volvo-car-editor",z);window.customCards??=[];window.customCards.push({type:"volvo-car-card",name:"Volvo Car Card",description:"A card for displaying Volvo Cars integration data and controls.",preview:!1,documentationURL:"https://github.com/home-assistant/core/tree/dev/homeassistant/components/volvo"});
