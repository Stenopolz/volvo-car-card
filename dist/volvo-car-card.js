      ...
      <div class="card-root">
        ...
        <div class="card-content">
          ...
          <div class="card-header">
            ...
            <!--<span class="version-badge" title="${e.version_tooltip}">${F}</span>-->
          </div>

          <!-- Primary range metric -->
          ${this._renderRangeMetric(Y,w,L,b,e)}

          <!-- Energy status rows -->
          <div class="status-block">
            ${G?this._renderChargeRow(u,A):""}
            ${this._renderFuelRow(T,s)}
          </div>
        ...
        <!-- Vehicle hero image -->
        <div class="hero-zone">
          ${V?`<img class="car-image" src="${V}" alt="${H}" />`:`<div class="hero-placeholder">${I}</div>`}
        </div>
        ...
        <div class="actions-bar">
          ${n?`
            <div class="btn-wrap">
              <button class="circle-btn" id="btn-lock" title="${c?e.btn.unlock:e.btn.lock}">
                ${c?Q:j}
              </button>
              <span class="btn-label">${c?e.btn.unlock:e.btn.lock}</span>
            </div>
          `:""}
          ${a?`
            <div class="btn-wrap">
              <button class="circle-btn" id="btn-climate" title="${e.btn.climate}">
                ${X}
              </button>
              <span class="btn-label">${e.btn.climate}</span>
            </div>
          `:""}
          ${p?`
            <div class="btn-wrap">
              <button class="circle-btn" id="btn-engine-start" title="${e.btn.engine_start}">
                ${J}
              </button>
              <span class="btn-label">${e.btn.engine_start}</span>
            </div>
          `:""}
          ${d?`
            <div class="btn-wrap">
              <button class="circle-btn" id="btn-engine-stop" title="${e.btn.engine_stop}">
                ${W}
              </button>
              <span class="btn-label">${e.btn.engine_stop}</span>
            </div>
          `:""}
        </div>

      </div>
    `,this._attachHandlers(n,c,a,p,d)}_renderRangeMetric(e,o,i,n,l){let r=o!==null&&e!==null&&!isNaN(e),t=n!==null&&i!==null&&!isNaN(i);if(r&&t)return`
        <div class="range-metric range-metric--combined">
          <div class="range-combined-total">
            <span class="range-value">${(Math.round(i)+Math.round(e)).toLocaleString()}</span>
          ...
          <div class="range-breakdown">
            ...
            <span class="range-part">
              <span class="range-part-icon range-part-icon--electric">${g}</span>
              ${Math.round(e)}
            </span>
            <span class="range-part-unit">km</span>
          </div>
        </div>
      `;if(!o&&!n)return'<div class="range-metric"><span class="range-unavailable">— km</span></div>';let a=r?e:t?i:null;if(a===null)return`<div class="range-metric"><span class="range-unavailable">${l.updating}</span></div>`;let p=r?g:x,d=r?"range-part-icon--electric":"range-part-icon--fuel";return`
      <div class="range-metric">
        <span class="range-value">${Math.round(a).toLocaleString()}</span>
        <span class="range-unit">km</span>
        <span class="range-part-icon ${d} range-single-icon">${p}</span>
      </div>
    `}_renderChargeRow(e,o){let i=e!==null?`${Math.round(e)}%`:"—";return`
      <div class="status-row">
        <span class="status-icon">${g}</span>
        <span class="status-pct">${i}</span>
        ${this._renderSegBar(e,"green")}
        <span class="status-label">${o}</span>
      </div>
    `}_renderFuelRow(e,o){if(!this._config?.fuel_entity)return"";let i=o?.attributes.unit_of_measurement??"L",n=e!==null&&!isNaN(e)?`${Math.round(e)} ${i}`:"—";return`
      <div class="status-row status-row--fuel">
        <span class="status-icon">${x}</span>
        <span class="status-pct">${n}</span>
      </div>
    `}_renderSegBar(e,o){let i=e!==null&&!isNaN(e)?Math.round(e/(100/Z)):0;return`<div class="seg-bar">${Array.from({length:Z},(l,r)=>`<span class="seg ${r<i?o:""}"></span>`).join("")}</div>`}_energyBgStyle(e){let o=Math.max(0,Math.min(100,e)),i=`${Math.max(0,o-1)}%`,n=`${o}%`;return`background: linear-gradient(to right,
      rgba(74,222,128,0.28) 0%,
      rgba(74,222,128,0.28) ${i},
      rgba(74,222,128,0.06) ${n},
      transparent 100%);`}_chargingLabel(e,o,i){if(!e){if(o!==null&&o>=100)return i.state.fully_charged;return i.state.ready_to_drive}switch(e.state.toLowerCase().trim()){case"idle":return i.state.idle;case"charging":return i.state.charging;case"scheduled":return i.state.scheduled;case"discharging":return i.state.discharging;case"error":return i.state.error;case"done":return i.state.done;default:return e.state||i.state.ready_to_drive}}_getState(e){if(!e||!this._hass)return null;return this._hass.states[e]??null}_deriveName(e,o){if(!e)return o;return(e.attributes.friendly_name??"").replace(/\s+(battery.*|lock.*|odometer.*|distance.*|fuel.*|charging.*|camera.*)$/i,"").trim()||o}_statesChanged(e,o){if(!e||!this._config)return!0;return[this._config.battery_entity,this._config.battery_range_entity,this._config.lock_entity,this._config.fuel_entity,this._config.fuel_range_entity,this._config.charging_status_entity,this._config.climate_entity,this._config.engine_start_entity,this._config.engine_stop_entity,this._config.vehicle_image_entity].filter(Boolean).some((n)=>e.states[n]!==o.states[n])}_attachHandlers(e,o,i,n,l){let r=this.shadowRoot;r.getElementById("btn-lock")?.addEventListener("click",()=>{if(!e)return;let t=o?"unlock":"lock";this._callService("lock",t,e)}),r.getElementById("btn-climate")?.addEventListener("click",()=>{if(!i)return;let t=i.split(".")[0],a=t==="button"?"press":"turn_on";this._callService(t,a,i)}),r.getElementById("btn-engine-start")?.addEventListener("click",()=>{if(!n)return;this._callService("button","press",n)}),r.getElementById("btn-engine-stop")?.addEventListener("click",()=>{if(!l)return;this._callService("button","press",l)})}_callService(e,o,i){if(!this._hass)return;this._hass.callService(e,o,{entity_id:i}).catch((n)=>{console.error(`[volvo-car-card] Service call ${e}.${o} failed:`,n)})}}var _=[{name:"name",selector:{text:{}}},{name:"battery_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"battery_range_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"fuel_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"fuel_range_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"lock_entity",selector:{entity:{filter:[{domain:"lock"}]}}},{name:"charging_status_entity",selector:{entity:{filter:[{domain:"sensor"}]}}},{name:"climate_entity",selector:{entity:{filter:[{domain:"button"}]}}},{name:"engine_start_entity",selector:{entity:{filter:[{domain:"button"}]}}},{name:"engine_stop_entity",selector:{entity:{filter:[{domain:"button"}]}}},{name:"vehicle_image_entity",selector:{entity:{filter:[{domain:"camera"},{domain:"image"}]}}},{name:"vehicle_image_url",selector:{text:{}}}];class y extends HTMLElement{_config={type:"custom:volvo-car-card"};_hass;_built=!1;_form;set hass(e){if(this._hass=e,this._form)this._form.hass=e,this._form.computeLabel=this._makeComputeLabel()}setConfig(e){if(this._config={...e},this._form)this._form.data=this._config}_makeComputeLabel(){let e=h(this._hass?.language??"en").editor;return(o)=>e[o.name]??o.name}connectedCallback(){if(this._built)return;this._built=!0,this.attachShadow({mode:"open"});let e=this.shadowRoot;e.innerHTML=`<style>
      ha-form { display: block; }
    </style>`;let o=document.createElement("ha-form");this._form=o,o.hass=this._hass,o.data=this._config,o.schema=_,o.computeLabel=this._makeComputeLabel(),o.addEventListener("value-changed",(i)=>{let n={...i.detail.value,type:"custom:volvo-car-card"};this._config=n,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:n},bubbles:!0,composed:!0}))}),e.appendChild(o)}}customElements.define("volvo-car-card",z);customElements.define("volvo-car-editor",y);window.customCards??=[];window.customCards.push({type:"volvo-car-card",name:"Volvo Car Card",description:"A card for displaying Volvo Cars integration data and controls.",preview:!1,documentationURL:"https://github.com/home-assistant/core/tree/dev/homeassistant/components/volvo"});
